...
const areas = [
  ...
  'Manchester', 'Birmingham', 'Leeds', 'Sheffield', 'Bristol', 'Edinburgh',
];
        ...
        <div className="grid lg:grid-cols-3 gap-12 items-center">
          <div>
            ...
            <h2 className="heading-display !text-white text-3xl lg:text-4xl mb-3">
              London &amp; Nationwide UK
            </h2>
            <p className="text-ink-300 leading-relaxed mb-5">
              We cover every London borough and offer nationwide UK removals.
              Whether you're moving across town or across the country, we've got you covered.
              Need a long-distance move? <Link href="/custom-quote" className="text-ember-400 hover:underline">Request a custom quote</Link>.
            </p>
            <Link href="/booking" className="btn-primary">Book your move</Link>
          </div>
          <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-sm">
            {areas.map((a) => (
              <div key={a} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition">
                <span className="text-ember-400 mr-1">·</span>{a}
              </div>
            ))}
            <div className="px-3 py-2 rounded-xl bg-ember-500/20 border border-ember-500/30 text-ember-300 font-semibold col-span-2 sm:col-span-1 text-center">
              + many more
            </div>
          </div>
        </div>
